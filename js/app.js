// Must wrap top-level as an async function to work on mobile
(async () => {
  ...
  if(await user.validate()) {
    ...
    /**
     * Render the market status section of the page. Only show the markets of the strategies that 
     * the user is licensed to. OHLC, volume, and %change over various periods is shown.
     */ 
    ...
    today['strategy-status'].forEach((strategy, index) => {
      let status = new Render('strategy-status', document.querySelector('main'), strategy, user);
      status.stylise();
    })
    ...
    /**
     * Unauthorised access. There was either no id provided or an unregistered id, so the user is 
     * not permitted to access the private data on this site.
     */
