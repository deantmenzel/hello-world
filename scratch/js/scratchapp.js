...
// Must use strict declaration of async functions to work on mobile
(async () => {

  const { User } = await import('./scratchuser.mjs');
  // import { User } from './scratchuser.mjs';
  const user = new User();

  const { Render } = await import('./scratchrender.mjs');
  // import { Render } from './scratchrender.mjs';
  Render.initialise();
...
if(await user.validate()) {
   ...
   */
  const { DB } = await import('./scratchdb.mjs');
  await DB.build();
